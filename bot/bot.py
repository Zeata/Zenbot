...
async def on_member_join(member):
    channel = client.get_channel(870490634870476881)
    await channel.send(f'Welcome {member.mention}')

@client.event
async def on_member_remove(member):
    channel = client.get_channel(870490634870476881)
    await channel.send(f'{member.mention} has fucked off')
#-----------------------------------------------
@client.command()
...
async def changeprefix_error(ctx, error):
    await ctx.send(f'{ctx.author.mention} you do not have permissions to change this prefix.')
#-----------------------------------------------
@client.command()
async def ping(ctx):
    await ctx.send('Pong!')
